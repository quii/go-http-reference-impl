...
import (
	...
	recipe_handler2 "github.com/quii/go-http-reference-impl/internal/adapters/http/internal/recipe_handler"
...
func TestGetRecipe(t *testing.T) {
	...
	t.Run("gets recipe by id", func(t *testing.T) {
		...
		router.HandleFunc("/recipes/{id}", recipe_handler2.NewRecipeHandler(stubService).GetRecipe)
	...
	t.Run("stores recipes", func(t *testing.T) {
		...
		dto := recipe_handler2.RecipeDTO{
			Ingredients: recipe.Ingredients,
			Directions:  recipe.Directions,
			Name:        recipe.Name,
		}
		handler := recipe_handler2.NewRecipeHandler(spyService)
